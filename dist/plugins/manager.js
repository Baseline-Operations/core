...
export class PluginManager {
        ...
        try {
            ...
            if (workspaceRoot) {
                ...
                if (config?.plugins?.dependencies) {
                    const { PluginInstaller } = await import("../utils/plugin-installer.js");
                    const lock = await PluginInstaller.loadLock(workspaceRoot);
                    // Install missing plugins
                    for (const [pluginId, dependency] of Object.entries(config.plugins.dependencies)) {
                        if (!lock.plugins[pluginId]) {
                            try {
                                Logger.debug(`Auto-installing plugin ${pluginId}...`);
                                const installed = await PluginInstaller.install(workspaceRoot, pluginId, dependency);
                                lock.plugins[installed.id] = installed;
                                await PluginInstaller.saveLock(workspaceRoot, lock);
                            }
                            catch (error) {
                                Logger.warn(`Failed to auto-install plugin ${pluginId}: ${error instanceof Error ? error.message : String(error)}`);
                            }
                        }
                    }
                }
                ...
                if (config?.repos) {
                    ...
                    for (const repo of normalizedRepos) {
                        ...
                        if (projectConfig?.requiredPlugins) {
                            for (const [pluginId, dependency,] of Object.entries(projectConfig.requiredPlugins)) {
                                if (!lock.plugins[pluginId] &&
                                    !this.plugins.has(pluginId)) {
                                    try {
                                        Logger.debug(`Auto-installing plugin ${pluginId} required by ${repo.name}...`);
                                        const installed = await PluginInstaller.install(workspaceRoot, pluginId, dependency);
                                        ...
                                        Logger.warn(`Failed to auto-install plugin ${pluginId} required by ${repo.name}: ${error instanceof Error ? error.message : String(error)}`);
                                    }
                                }
                            }
                        }
                        ...
                        if (repo.requiredPlugins) {
                            for (const pluginId of repo.requiredPlugins) {
                                if (!lock.plugins[pluginId] &&
                                    !this.plugins.has(pluginId)) {
                                    Logger.warn(`Repository ${repo.name} requires plugin ${pluginId} but it's not installed. Install it with: baseline plugin install ${pluginId}`);
                                }
                            }
                        }
                    }
     ...
     */
    getLanguagePlugins() {
        return this.getPluginsByType("language");
    }
    /**
     * Get language profile from a plugin.
     */
    getLanguageProfile(pluginId, options) {
        const plugin = this.getPlugin(pluginId);
        if (!plugin || plugin.metadata.type !== "language") {
            return null;
        }
        return plugin.getLanguageProfile(options);
    }
    /**
    ...
    async loadBuiltInPlugins() {
        try {
            const { registerBuiltInPlugins } = await import("./builtin/index.js");
     ...
     */
    async checkBaselineVersion(requiredVersion) {
        if (!requiredVersion) {
        ...
        try {
            ...
            if (existsSync(packageJsonPath)) {
                ...
                if (requiredVersion.startsWith("^") ||
                    ...
                    const requiredMajor = parseInt(requiredBase.split(".")[0] || "0");
     ...
     */
    checkRequiredLanguages(requiredLanguages) {
     ...
     */
    async discoverExternalPlugins(workspaceRoot) {
        if (!workspaceRoot) {
            return;
        }
        // Load plugins from installed plugins directory (.baseline/.plugins/)
        const installedPluginsDir = join(workspaceRoot, ".baseline", ".plugins");
            ...
            try {
                ...
                for (const entry of entries) {
                    if (entry.isDirectory()) {
                        await this.loadPluginFromDirectory(join(installedPluginsDir, entry.name), entry.name);
                    }
                        ...
                        if (ext === ".js" ||
                            ext === ".mjs" ||
                            ext === ".cjs") {
                            await this.loadPluginFromFile(join(installedPluginsDir, entry.name), entry.name);
                        }
                    }
                }
            }
            catch (error) {
                Logger.warn(`Failed to read installed plugins directory: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
            ...
            try {
                ...
                for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        loadPromises.push(this.loadPluginFromDirectory(entryPath, entry.name));
                    }
                    else if (entry.isFile()) {
                        // Single plugin file
                        const ext = extname(entry.name);
                        if (ext === ".js" ||
                            ext === ".mjs" ||
                            ext === ".cjs") {
                            loadPromises.push(this.loadPluginFromFile(entryPath, entry.name));
                        }
                    }
                }
                ...
                Logger.warn(`Failed to read plugins directory: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        // Discover and load plugins from npm packages
        try {
            const { NpmPluginDiscovery } = await import("../utils/npm-plugin-discovery.js");
            const npmPlugins = await NpmPluginDiscovery.discoverFromDependencies(workspaceRoot);
            for (const npmPlugin of npmPlugins) {
                try {
                    // Plugin is already installed in node_modules
                    // Try to load it
                    const nodeModulesPath = join(workspaceRoot, "node_modules", npmPlugin.name);
                    const packageJsonPath = join(nodeModulesPath, "package.json");
                    if (existsSync(packageJsonPath)) {
                        const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf-8"));
                        const pluginEntry = packageJson.baselinePlugin || "index.js";
                        const pluginPath = join(nodeModulesPath, pluginEntry);
                        if (existsSync(pluginPath)) {
                            const pluginUrl = pathToFileURL(pluginPath).href;
                            const pluginModule = await import(`${pluginUrl}?${Date.now()}`);
                            const pluginOrPackage = pluginModule.default ||
                                pluginModule.plugin ||
                                pluginModule.package;
                            if (this.isValidPluginPackage(pluginOrPackage)) {
                                await this.registerPluginPackage(pluginOrPackage, `npm:${npmPlugin.name}`);
                            }
                            else if (this.isValidPlugin(pluginOrPackage)) {
                                await this.registerPluginWithChecks(pluginOrPackage, `npm:${npmPlugin.name}`);
                            }
                        }
                    }
                }
                catch (error) {
                    Logger.debug(`Failed to load npm plugin ${npmPlugin.name}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
        catch (error) {
            Logger.debug(`Failed to discover npm plugins: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Load a single plugin from a file.
     */
    async loadPluginFromFile(filePath, fileName) {
        try {
            ...
            if (this.isValidPluginPackage(pluginOrPackage)) {
                ...
                await this.registerPluginPackage(pluginOrPackage, fileName);
            }
            else if (this.isValidPlugin(pluginOrPackage)) {
                // It's a single plugin
                await this.registerPluginWithChecks(pluginOrPackage, fileName);
            }
            else {
                Logger.warn(`Invalid plugin format in ${fileName}: missing required metadata or structure`);
            }
        }
        catch (error) {
            Logger.warn(`Failed to load external plugin ${fileName}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Load a plugin package from a directory.
     */
    async loadPluginFromDirectory(dirPath, dirName) {
        ...
        if (existsSync(packageJsonPath)) {
            try {
                ...
                if (packageJson.main) {
                    possibleEntries.unshift(join(dirPath, packageJson.main));
                }
            }
            catch {
     ...
     */
    async registerPluginWithChecks(plugin, source) {
        // Check baseline version requirement
        if (!(await this.checkBaselineVersion(plugin.metadata.baselineVersion))) {
            Logger.warn(`Plugin ${plugin.metadata.id} requires baseline version ${plugin.metadata.baselineVersion}, skipping`);
            return;
        }
        // Check dependencies
        const depCheck = this.checkDependencies(plugin.metadata.requires);
        if (!depCheck.satisfied) {
            Logger.warn(`Plugin ${plugin.metadata.id} requires missing plugins: ${depCheck.missing.join(", ")}, skipping`);
            return;
        }
        // Check required languages
        const langCheck = this.checkRequiredLanguages(plugin.metadata.requiresLanguages);
        if (!langCheck.satisfied) {
            Logger.warn(`Plugin ${plugin.metadata.id} requires missing languages: ${langCheck.missing.join(", ")}, skipping`);
            return;
        }
        // Check if plugin is already registered (don't overwrite built-ins)
        if (!this.plugins.has(plugin.metadata.id)) {
            this.registerPlugin(plugin);
            Logger.debug(`Loaded external plugin: ${plugin.metadata.id} from ${source}`);
        }
        else {
            Logger.debug(`Skipping external plugin ${plugin.metadata.id} (already registered as built-in)`);
        }
    }
    /**
     * Register a plugin package (multiple plugins).
     */
    async registerPluginPackage(pluginPackage, source) {
        // Check baseline version requirement
        if (!(await this.checkBaselineVersion(pluginPackage.metadata.baselineVersion))) {
            Logger.warn(`Plugin package ${pluginPackage.metadata.name} requires baseline version ${pluginPackage.metadata.baselineVersion}, skipping`);
            return;
        }
        // Check package dependencies
        const depCheck = this.checkDependencies(pluginPackage.dependencies);
        if (!depCheck.satisfied) {
            Logger.warn(`Plugin package ${pluginPackage.metadata.name} requires missing plugins: ${depCheck.missing.join(", ")}, skipping`);
            return;
        }
        // Check required languages
        const langCheck = this.checkRequiredLanguages(pluginPackage.requiresLanguages);
        if (!langCheck.satisfied) {
            Logger.warn(`Plugin package ${pluginPackage.metadata.name} requires missing languages: ${langCheck.missing.join(", ")}, skipping`);
            return;
        }
        // Register all plugins in the package
        for (const plugin of pluginPackage.plugins) {
            await this.registerPluginWithChecks(plugin, `${pluginPackage.metadata.name} (from ${source})`);
        }
    }
    /**
     * Validate that an object is a valid plugin package.
     */
    isValidPluginPackage(obj) {
        return (obj &&
            typeof obj === "object" &&
            obj.metadata &&
            typeof obj.metadata.name === "string" &&
            ...
            obj.plugins.every((p) => this.isValidPlugin(p)));
    }
}
