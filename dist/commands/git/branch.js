 ...
 */
export async function branchRepositories(branchName, options = {}) {
    const messages = [];
    const errorRepos = [];
    const skippedRepos = [];
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const { stdout } = await execa("git", ["branch", "-r", "--list", `${remote}/${branchName}`], { cwd: repoPath });
                    branchExists = stdout.trim().length > 0;
                    if (branchExists) {
                        // Branch exists on remote, checkout with tracking
                        await execa("git", ["checkout", "-b", branchName, `${remote}/${branchName}`], { cwd: repoPath, stdio: "ignore" });
                        checkedOut++;
                        messages.push({
                            type: "success",
