 ...
 */
export async function createPullRequests(options = {}) {
    const messages = [];
    const errorRepos = [];
    const skippedRepos = [];
    const prUrls = [];
    ...
    const providerPlugins = pluginManager.getPluginsByType("provider");
    let providerPlugin;
    ...
    for (const repo of reposToProcess) {
        try {
            ...
            try {
                ...
                const { stdout } = await execa("git", ["rev-list", "--count", `${remote}/${currentBranch}`], { cwd: repoPath });
            ...
            const prTitle = options.title ||
                ...
                `PR for ${repo.name}`;
            const prBody = options.body || `Updates from baseline workspace.`;
            try {
