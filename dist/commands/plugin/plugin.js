...
export async function installPlugin(pluginId, options = {}) {
    ...
    try {
        ...
        const pluginPath = await PluginInstaller.getPluginPath(workspaceRoot, installed.id);
        if (pluginPath) {
 ...
 * This is a pure function that returns results - no logging or process.exit.
 */
export async function removePlugin(pluginId, options = {}) {
    const messages = [];
    let workspaceRoot = options.workspaceRoot;
    const configManager = workspaceRoot ? new ConfigManager(workspaceRoot) : new ConfigManager();
    if (!workspaceRoot) {
    ...
    try {
        ...
        if (options.removeFromConfig !== false &&
            config.plugins?.dependencies) {
...
export async function installAllPlugins(options = {}) {
    ...
    for (const [pluginId, dependency] of Object.entries(config.plugins.dependencies)) {
        try {
            ...
            const installedPlugin = await PluginInstaller.install(workspaceRoot, pluginId, dependency);
            // Update lock file
            const lock = await PluginInstaller.loadLock(workspaceRoot);
...
export async function searchPlugins(query, options = {}) {
    ...
    if (options.registry) {
        ...
        try {
            const { PluginRegistryClient } = await import("../../utils/plugin-registry.js");
            const results = await PluginRegistryClient.search(workspaceRoot, query);
            if (results.length === 0) {
