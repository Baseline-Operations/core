 ...
 */
export async function linkRepositories(options = {}) {
    const messages = [];
    const linkedConfigs = [];
    ...
    for (const repo of normalizedRepos) {
        const detectedLanguages = repo.languages && repo.languages.length > 0 ?
            repo.languages
            : await LanguageDiscovery.detectLanguages(join(workspaceRoot, repo.path), pluginManager);
    ...
    for (const [langId, repos] of reposByLanguage.entries()) {
        ...
        for (const pmPlugin of compatiblePMs) {
            ...
            try {
                const workspaceConfig = await pmPlugin.createWorkspaceConfig(reposForConfig, workspaceRoot);
                if (workspaceConfig) {
