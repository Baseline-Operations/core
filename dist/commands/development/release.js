 ...
 */
export async function releaseCommand(subcommand, options = {}) {
...
async function releasePlan(configManager, config) {
    const messages = [];
    ...
    for (const repo of normalizedRepos) {
        if (repo.languages?.includes("node")) {
            ...
            const detectedLanguages = await LanguageDiscovery.detectLanguages(join(workspaceRoot, repo.path), pluginManager);
    ...
    for (const repo of nodeRepos) {
        const packageJsonPath = join(workspaceRoot, repo.path, "package.json");
        ...
        try {
            ...
            const status = (await GitUtil.isRepo(repoPath)) ?
...
async function releaseVersion(configManager, config) {
    const messages = [];
    const packages = [];
    const workspaceRoot = configManager.getWorkspaceRoot();
...
async function releasePublish(configManager, config) {
    const messages = [];
    const packages = [];
    const workspaceRoot = configManager.getWorkspaceRoot();
