 ...
 */
export async function configRepositories(options = {}) {
    const messages = [];
    const errorRepos = [];
    const skippedRepos = [];
    const configFiles = [];
    let workspaceRoot = options.workspaceRoot;
    const configManager = workspaceRoot ?
    ...
    if (!config) {
        return {
            ...
            messages: [
                {
                    type: "error",
                    message: "No baseline workspace found. Run `baseline init` first.",
                },
            ],
            errorRepos: [],
            skippedRepos: [],
    ...
    const reposToProcess = options.repo ?
    ...
    for (const repo of reposToProcess) {
        try {
            ...
            const testCommand = await LanguageDiscovery.discoverCommand(repo, workspaceRoot, "test", pluginManager);
            if (testCommand)
                defaultCommands.test = testCommand;
            const lintCommand = await LanguageDiscovery.discoverCommand(repo, workspaceRoot, "lint", pluginManager);
            if (lintCommand)
                defaultCommands.lint = lintCommand;
            ...
            const projectConfig = configExists && !options.force ?
            ...
            const finalCommands = {};
            if (commands.test)
                finalCommands.test = commands.test;
            if (commands.lint)
                finalCommands.lint = commands.lint;
            if (commands.start)
                finalCommands.start = commands.start;
            // Build project config
            const newProjectConfig = {
                name: repo.name,
                library: repo.library ?? projectConfig.library ?? false,
                commands: Object.keys(finalCommands).length > 0 ?
                    finalCommands
                    : undefined,
            };
    ...
    if (config.editor) {
        ...
        const editorIds = typeof config.editor === "string" ?
            [config.editor]
            : config.editor;
