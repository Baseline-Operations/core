...
export async function generateGraph(options = {}) {
    ...
    for (const repo of normalizedRepos) {
        ...
        const dependencies = await detectDependencies(repo, repoPath, normalizedRepos, workspaceRoot);
 ...
 */
async function detectDependencies(repo, repoPath, allRepos, workspaceRoot) {
    const dependencies = [];
