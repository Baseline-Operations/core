...
import { PackageManagerUtil, } from "../../utils/package-manager.js";
...
export async function initWorkspace(options = {}) {
    ...
    try {
        ...
        const defaultPM = detectedPM && pmInstalled.has(detectedPM) ?
            detectedPM
            : pmChoices[0].name;
        ...
        for (const langId of answers.languages) {
            ...
            if (languagePlugin.getPrompts && langId === "node") {
                ...
                const pmPrompt = pluginPrompts.find((p) => p.name === "pmVersionType");
                if (pmPrompt) {
                    pmPrompt.message = `${answers.packageManager} version policy`;
                }
                const pmVersionInput = pluginPrompts.find((p) => p.name === "pmVersion");
                if (pmVersionInput) {
                    pmVersionInput.message = `${answers.packageManager} version`;
                }
                const nodeVersionRules = await prompt(pluginPrompts);
                if (nodeVersionRules.nodeVersionType !== "skip" &&
                    nodeVersionRules.nodeVersion) {
                    versionPolicies.node = {
                        [nodeVersionRules.nodeVersionType]: nodeVersionRules.nodeVersion,
                    };
                }
                if (nodeVersionRules.pmVersionType !== "skip" &&
                    nodeVersionRules.pmVersion) {
                    versionPolicies[answers.packageManager] = {
                        [nodeVersionRules.pmVersionType]: nodeVersionRules.pmVersion,
                    };
                }
                if (nodeVersionRules.tsVersionType !== "skip" &&
                    nodeVersionRules.tsVersion) {
                    versionPolicies.typescript = {
                        [nodeVersionRules.tsVersionType]: nodeVersionRules.tsVersion,
                    };
                }
            }
            // Get language profile from plugin
            const profile = languagePlugin.getLanguageProfile({
                packageManager: langId === "node" ? answers.packageManager : undefined,
                versionPolicies: Object.keys(versionPolicies).length > 0 ?
                    versionPolicies
                    : undefined,
            });
        ...
        const languagesConfigSimplified = {};
        ...
        const config = {
            ...
            editor: (() => {
                const editors = [];
                if (answers.generateVSCode)
                    editors.push("vscode");
                if (answers.generateCursor)
                    editors.push("cursor");
                if (answers.generateIntelliJ)
                    editors.push("intellij");
                if (answers.generateSublime)
        ...
        if (config.editor) {
            ...
            for (const editorPlugin of editorPlugins) {
                const ep = editorPlugin;
                const shouldGenerate = editorIds.includes(ep.metadata.id);
                if (shouldGenerate && ep.generateWorkspaceFile) {
                    const workspaceFile = await ep.generateWorkspaceFile(config, workspaceRoot);
                    if (workspaceFile) {
                        // Create directory if needed (e.g., .idea for IntelliJ)
                        const targetPath = join(workspaceRoot, workspaceFile.file);
...
async function generateGitignore(root) {
