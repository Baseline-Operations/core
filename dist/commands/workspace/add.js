 ...
 */
export async function addRepository(gitUrl, options = {}) {
    ...
    const hasCommands = commands && (commands.test || commands.lint || commands.start);
    const newRepo = {
        id: repoName,
        name: repoName,
        location: gitUrl,
    ...
    if (normalizedRepos.some((r) => r.name === repoName || r.path === repoPath)) {
        return {
            success: false,
            repo: newRepo,
            errors: [
