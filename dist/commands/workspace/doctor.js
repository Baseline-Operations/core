...
export async function doctorCheck(options = {}) {
    ...
    if (config.languages) {
        for (const [toolName, versionString] of Object.entries(config.languages)) {
            ...
            try {
                ...
                const { stdout } = await execa(detection.command, detection.args || [], {
                    timeout: 5000,
                });
    ...
    if (pmPlugin &&
        ...
        pmPlugin.isInstalled) {
        const packageManagerPlugin = pmPlugin;
        ...
        const pmInstalled = await PackageManagerUtil.isInstalled(defaultPMId);
        if (!pmInstalled) {
            const msg = `Package manager ${defaultPMId} is not installed`;
            issues.push({
                severity: "error",
            ...
            const pmVersion = await PackageManagerUtil.getVersion(defaultPMId);
    ...
    for (const repo of normalizedRepos) {
        ...
        const projectFiles = await LanguageDiscovery.getProjectFiles(repo, workspaceRoot, pluginManager);
        ...
        if (repo.languages?.includes("node")) {
            const repoPM = repo.packageManager ||
            ...
            const hasLockfile = existsSync(join(repoPath, expectedLockfile));
            if (!hasLockfile) {
