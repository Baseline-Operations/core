 ...
 */
export async function dockerCompose(subcommand, options = {}) {
    const messages = [];
    const failedRepos = [];
    ...
    for (const compose of composeFiles) {
        try {
            ...
            const args = buildDockerComposeArgs(subcommand, compose.file, options, compose.path);
 ...
 */
async function checkDockerComposePlugin() {
    try {
        await execa("docker", ["compose", "version"], { timeout: 5000 });
        return true;
    }
 ...
 */
function buildDockerComposeArgs(subcommand, composeFile, options, _composePath) {
    const args = [];
