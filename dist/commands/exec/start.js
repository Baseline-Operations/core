 ...
 */
export async function startApplications(options = {}) {
    const messages = [];
    const skippedRepos = [];
    const errorRepos = [];
    ...
    for (const repo of repos) {
        ...
        const startCommand = await getRepoStartCommand(repo, workspaceRoot);
        if (!startCommand) {
        ...
        try {
            ...
            if (repo.startInDocker) {
                await startInDocker(repo, workspaceRoot, startCommand, commandRunner.runner || "npm", messages);
            }
            else {
                const [cmd, ...args] = startCommand.split(/\s+/);
                if (commandRunner.runner) {
                    // Run in background (don't wait for it)
                    execa(commandRunner.runner, [...commandRunner.args, cmd, ...args], {
                        cwd: repoPath,
                        stdio: "inherit",
                        detached: true,
                    });
...
async function getRepoStartCommand(repo, workspaceRoot) {
    ...
    return await LanguageDiscovery.discoverCommand(repo, workspaceRoot, "start", pluginManager);
}
async function startInDocker(repo, workspaceRoot, startCommand, repoPM, messages) {
    const repoPath = join(workspaceRoot, repo.path);
