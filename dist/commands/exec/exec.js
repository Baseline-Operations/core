...
async function execSequential(repos, command, workspaceRoot, failFast, messages, failedRepos) {
    let completed = 0;
    let failed = 0;
...
async function execParallel(repos, command, workspaceRoot, failFast, messages, failedRepos) {
    const [cmd, ...args] = command.split(/\s+/);
