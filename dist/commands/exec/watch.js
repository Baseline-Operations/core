 ...
 */
export async function watchRepositories(options = {}) {
    const messages = [];
    const failedRepos = [];
    const skippedRepos = [];
    const watchers = [];
    ...
    for (const repo of repos) {
        ...
        try {
            const watchConfig = getRepoWatchConfig(repo, workspaceRoot);
            // Get ignore patterns
            ...
            const handleFileChange = async (filePath) => {
                ...
                if (watchConfig?.patterns) {
                    if (!matchesPattern(relativePath, watchConfig.patterns)) {
                        return;
                    }
                }
                ...
                debounceTimeout = setTimeout(async () => {
                    ...
                    try {
                        const currentConfig = await configManagerRef.load();
                        if (currentConfig) {
                            if (customCommand) {
                                await runCustomWatchCommand(repo, workspaceRoot, currentConfig, customCommand);
                            }
                            else {
                                await runRepoTests(repo, workspaceRoot, currentConfig);
                            }
                        }
                    }
                    catch (error) {
            ...
            if (chokidar) {
                ...
                watcher = watch(repoPath, { recursive: true }, async (eventType, filename) => {
                    if (!filename)
                        return;
                    const filePath = filename.startsWith("/") ? filename : (join(repoPath, filename));
...
async function runRepoTests(repo, workspaceRoot, _config) {
...
function getRepoWatchConfig(repo, workspaceRoot) {
    const repoPath = join(workspaceRoot, repo.path);
...
async function runCustomWatchCommand(repo, workspaceRoot, config, command) {
    const repoPath = join(workspaceRoot, repo.path);
