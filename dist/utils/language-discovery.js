...
export class LanguageDiscovery {
     ...
     */
    static async discoverCommand(repo, workspaceRoot, commandType, pluginManager) {
            ...
            for (const plugin of allLanguagePlugins) {
                if (plugin.detectLanguage) {
                    ...
                    if (isMatch && plugin.discoverCommands) {
                        const discovery = await plugin.discoverCommands(repoPath);
                        if (discovery && discovery[commandType]) {
                            return discovery[commandType];
                        }
                    }
     ...
     */
    static async getProjectFiles(repo, workspaceRoot, pluginManager) {
        const repoPath = join(workspaceRoot, repo.path);
        const pm = pluginManager || PluginManager.getInstance();
        const files = [];
        if (repo.languages && repo.languages.length > 0) {
            for (const langId of repo.languages) {
                ...
                if (plugin &&
                    plugin.metadata.type === "language" &&
                    plugin.getProjectFiles) {
                    const languagePlugin = plugin;
                    if (languagePlugin.getProjectFiles) {
                        const projectFiles = languagePlugin.getProjectFiles();
                        if (projectFiles) {
     ...
     */
    static async detectLanguages(repoPath, pluginManager) {
        const pm = pluginManager || PluginManager.getInstance();
     ...
     */
    static async getCommandRunner(repo, workspaceRoot, configManager, pluginManager) {
        const pm = pluginManager || PluginManager.getInstance();
        const repoPath = join(workspaceRoot, repo.path);
            ...
            for (const langId of repo.languages) {
                ...
                if (plugin && plugin.metadata.type === "language") {
                    ...
                    if (languagePlugin.getCommandRunner) {
                        ...
                        const packageManager = langId === "node" && config ?
                            (config.packageManager || "npm")
                            : repo.packageManager;
                        const runner = await languagePlugin.getCommandRunner(repoPath, { packageManager });
                        if (runner) {
                            return runner;
                        }
                    }
            ...
            for (const plugin of allLanguagePlugins) {
                if (plugin.detectLanguage) {
                    ...
                    if (isMatch && plugin.getCommandRunner) {
                        const config = await configManager.load();
                        const packageManager = plugin.metadata.id === "node" && config ?
                            (config.packageManager || "npm")
                            : undefined;
                        const runner = await plugin.getCommandRunner(repoPath, { packageManager });
                        if (runner) {
                            return runner;
                        }
                    }
...
//# sourceMappingURL=language-discovery.js.map