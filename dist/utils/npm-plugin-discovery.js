...
export class NpmPluginDiscovery {
    ...
    static async search(query) {
        try {
            ...
            const { stdout } = await execa("npm", ["search", "--json", searchQuery], {
                timeout: 30000,
                stdio: "pipe",
            });
            ...
            return results
                .filter((pkg) => pkg.keywords?.includes("baseline-plugin"))
                .map((pkg) => ({
                name: pkg.name,
                version: pkg.version,
            ...
            Logger.warn(`Failed to search npm: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
    /**
     ...
     */
    static async getPluginInfo(packageName, workspaceRoot) {
        try {
            ...
            const { stdout } = await execa("npm", ["view", packageName, "name", "version", "description", "keywords", "baselinePluginId", "baselinePlugin", "--json"], {
                timeout: 10000,
                stdio: "pipe",
            });
            ...
            Logger.debug(`Failed to get npm plugin info for ${packageName}: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }
    }
    /**
     ...
     */
    static async discoverFromDependencies(workspaceRoot) {
        ...
        try {
            ...
            Logger.warn(`Failed to discover plugins from dependencies: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }
    }
}
//# sourceMappingURL=npm-plugin-discovery.js.map