...
export class Logger {
    ...
    static table(headers, rows) {
        const widths = headers.map((header, i) => {
            ...
            return Math.max(header.length, ...colValues.map((v) => v.length));
        });
        const separator = `  ${chalk.gray("│")}  `;
        const headerRow = chalk.bold(headers.map((h, i) => h.padEnd(widths[i])).join(separator)) + "\n";
        const separatorRow = chalk.gray("─".repeat(widths.reduce((a, b) => a + b, 0) +
