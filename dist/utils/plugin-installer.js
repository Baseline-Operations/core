...
export class PluginInstaller {
     ...
     */
    static async saveLock(workspaceRoot, lock) {
        const lockPath = this.getLockFile(workspaceRoot);
     ...
     */
    static async installFromNpm(workspaceRoot, packageName, version) {
        ...
        try {
            ...
            const pluginInstallPath = join(pluginDir, `${packageName}-${installedVersion}`);
     ...
     */
    static async installFromGit(workspaceRoot, gitUrl, version) {
        const pluginDir = this.getPluginDir(workspaceRoot);
        await mkdir(pluginDir, { recursive: true });
     ...
     */
    static async installFromLocal(workspaceRoot, localPath) {
        const pluginDir = this.getPluginDir(workspaceRoot);
        await mkdir(pluginDir, { recursive: true });
     ...
     */
    static async installFromRemote(workspaceRoot, url) {
        const pluginDir = this.getPluginDir(workspaceRoot);
        await mkdir(pluginDir, { recursive: true });
     ...
     */
    static async install(workspaceRoot, pluginId, dependency) {
        const source = dependency.source || this.inferSource(dependency);
        switch (source) {
            case "npm":
                return this.installFromNpm(workspaceRoot, dependency.url || pluginId, dependency.version);
            case "git":
                return this.installFromGit(workspaceRoot, dependency.url || dependency.path || "", dependency.version);
            case "local":
                return this.installFromLocal(workspaceRoot, dependency.path || dependency.url || "");
            case "remote":
     ...
     */
    static async listInstalled(workspaceRoot) {
     ...
     * Remove a plugin.
     */
    static async remove(workspaceRoot, pluginId) {
        const lock = await this.loadLock(workspaceRoot);
     ...
     */
    static async getPluginPath(workspaceRoot, pluginId) {
        const lock = await this.loadLock(workspaceRoot);
        const installed = lock.plugins[pluginId];
        if (!installed) {
            return null;
